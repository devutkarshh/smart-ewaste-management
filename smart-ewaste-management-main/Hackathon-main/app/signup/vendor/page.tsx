...
function VendorSignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        const message = encodeURIComponent(result.message || "Account created successfully! Please sign in.")
        router.push(`/login/vendor?message=${message}`)
      } else {
  ...
  return (
    <div className="w-full max-w-md relative z-10">
      <Card className="border-[#9ac37e]/20 shadow-2xl backdrop-blur-sm bg-card/95 dark:bg-card/90">
        <CardHeader>
          <CardTitle className="text-[#3e5f44] dark:text-[#9ac37e] text-2xl font-bold">E‑waste Vendor Sign Up</CardTitle>
          <CardDescription className="text-[#3e5f44]/70 dark:text-[#9ac37e]/70">Create your vendor account</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input 
                ...
                placeholder="e.g., GreenTech Recycling Solutions"
                required 
              />
            </div>
            <div className="grid gap-2">
            ...
            <div className="text-center mt-2">
              <p className="text-sm text-[#3e5f44]/70 dark:text-[#9ac37e]/70">
                ...
                <Link href="/login/vendor" className="text-[#3e5f44] dark:text-[#9ac37e] font-semibold hover:underline">
                  Sign in
                </Link>
              </p>
            </div>
