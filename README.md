...
- **QR Code-Based Tagging** – Assign QR codes to each e-waste item for tracking from reporting to disposal/recycling. The QR currently encodes the item detail URL generated by the Flask app.
...
- **💡 Machine Learning Module** – A trained Random Forest model predicts resale price when the saved model files are available; otherwise the app falls back to a rule-based depreciation estimate.
...
- The app runs with Flask's built-in server in local development.
- For Render or similar hosting, use a production WSGI server and set the start command accordingly.
- QR codes are generated from the app URL currently configured in `app.py`, so the base URL should be checked before deploying to a public domain.
- The ML price prediction feature is optional at runtime: if the saved model files are missing, the app still works using the built-in depreciation estimate.
